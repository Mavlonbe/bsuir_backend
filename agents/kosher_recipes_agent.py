...
class Agent(object):
    ...
    def search_recipes(self, ingredients: EventSerializer) -> dict:
        recipes = self.client.run(
            ...
            AND NOT EXISTS {
                ...
                WHERE excluded.name IN ["pork", "crab", "lobster", "oyster", "mussels", "squid", "octopus", "shark", "eel", "frog", "turtle"]
                }
    ...
    def trigger(cls, event: EventSerializer) -> (dict, None):
        if event.data['EventType'] == "Search_Recipe_By_Ingredients_Jevish":
